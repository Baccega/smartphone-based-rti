...
def calibrate(calibration_video, save_file_path):
    ...
    objp = np.zeros(
        (constants["CHESSBOARD_SIZE"][0] * constants["CHESSBOARD_SIZE"][1], 3),
        np.float32,
    )
    objp[:, :2] = np.mgrid[
        0 : constants["CHESSBOARD_SIZE"][0], 0 : constants["CHESSBOARD_SIZE"][1]
    ].T.reshape(-1, 2)
...
def main():
    ...
    calibrate(
        constants["CALIBRATION_CAMERA_MOVING_PATH"],
